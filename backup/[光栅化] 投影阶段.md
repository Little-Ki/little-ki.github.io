...
<details>
<summary>信息</summary>
什么是  $l$、$t$、$r$、$b$ 坐标以及如何计算它们在[针孔相机模型：实现虚拟针孔相机课程](https://www.scratchapixel.com/lessons/3d-basic-rendering/3d-viewing-pinhole-camera/implementing-virtual-pinhole-camera.html)中进行了解释。快速提醒一下，这取决于焦距（以毫米或英寸为单位）、胶片孔径宽度和高度（以毫米或英寸为单位）以及 z 近剪裁平面。
</details>
